...
1. Read `notes.txt` end to end and extract every topic as `{ title, definition, codeSample }`.
...
4. For each topic emit these sections, in order:
   ...
   - `### Code Sample` — the learner's sample **verbatim**, in a ` ```ts ` fence, with an inline
     comment showing the inferred/checked result
...
- All snippets must compile under `strict: true`.
