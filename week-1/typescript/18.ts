...
const items : Items = {name: "", price: 0}
const AskCustomer = () => {
    ...
    rl.question('Do you want to add items to your cart ? type Yes or No: ', (message: string) => {
        if(message.toLowerCase() === "yes" )
        {
          rl.question('What is the item name ? ', (name: string) => { 
            items.name = name
            rl.question('How much the price of the item ? ', (price:string) => {
...
const addtoCart = (items:Items):void => {
    rl.question(`Do you want to save this items: [ ${items.name} , ${items.price} ] to your cart? type Yes or No: `, (confirmMsg: string) => {
...
const removeToCart = (items: Items):void => {

    rl.question('Do you want to remove some items from the cart ? type Yes or No: ', (message: string) => {
        ...
        else
        {
         ...
         else
         {
             console.log(`Your current items in cart:`);
             console.log(cart);
             rl.question('Select the name of item you want to remove from your cart: ', (select_name: string) => {
